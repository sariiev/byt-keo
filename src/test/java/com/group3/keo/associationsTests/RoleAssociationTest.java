...
public class RoleAssociationTest {
    ...
    void setup() {
        ...
        community1 = new Community("Community1", CommunityTopic.ANIMALS, null);
        community2 = new Community("Community2", CommunityTopic.ART, null);
    }

    @Test
    void userJoinCommunityShouldCreateRoleInBothSides() {
        Role role = user1.joinCommunity(community1, RoleType.HEAD);

        Assertions.assertTrue(Role.getExtent().contains(role));
        Assertions.assertEquals(role, community1.getMembers().get(user1.getUsername()));
        Assertions.assertTrue(user1.getRoles().contains(role));
    }

    @Test
    void communityAddMemberShouldCreateRoleInBothSides() {
        Role role = community1.addMember(user1, RoleType.HEAD);
    ...
    void cannotAddTheSameUserTwice() {
        ...
        Assertions.assertThrows(IllegalStateException.class,
                () -> {
                    community1.addMember(user1, RoleType.HEAD);
                }
        );
    }
    ...
    void userCanJoinMultipleCommunities() {
        user1.joinCommunity(community1, RoleType.HEAD);
        user1.joinCommunity(community2, RoleType.HEAD);

        Assertions.assertEquals(2, Role.getExtent().size());
    ...
    void removingMemberFromCommunityShouldRemoveAllReferences() {
        ...
        community1.removeMember(user1);

        Assertions.assertFalse(Role.getExtent().contains(role));
        Assertions.assertFalse(community1.getMembers().containsKey(user1.getUsername()));
        Assertions.assertFalse(user1.getRoles().contains(role));
    }
    ...
    void userLeaveCommunityShouldRemoveAllReferences() {
        ...
        user1.leaveCommunity(community1);
    ...
    void deletingUserShouldRemoveTheirRoles() {
        ...
        Assertions.assertEquals(0, community2.getMembers().size());
    }

    @Test
    void deletingCommunityShouldRemoveAllItsRoles() {
        user1.joinCommunity(community1, RoleType.HEAD);
        user2.joinCommunity(community1, RoleType.HEAD);
    ...
    void usersUsernameChangeShouldUpdateCommunityKey() {
        ...
        Assertions.assertNotNull(community1.getMembers().get(user1.getUsername()));
    }
}
